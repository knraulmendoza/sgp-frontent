    ...
    >
        <template v-slot:top>
            <v-toolbar dark color="green">
                ...
                <v-dialog
                    v-model="dialogAprobarPropuesta"
                    persistent
                    scrollable
                    max-width="900px"
                >
        ...
        <template v-slot:item.verPDF="{ item }">
            <v-icon large @click="showPDF(item)" color="red"
                >mdi-file-pdf-box</v-icon
            >
        </template>
        <template v-slot:item.presupuestoEstimado="{ item }">
            <span>{{ parseFloat(item.presupuestoEstimado) | currency }}</span>
        </template>
        <template v-slot:item.actionAprobarPropuesta="{ item }">
            <v-icon large @click="aprobarPropuesta(item)" color="green"
                >mdi-check</v-icon
            >
        </template>
    </v-data-table>
