...
export default class AddSgp extends Vue {
    ...
    public archivoValor: Blob = new Blob([''], { type: 'application/pdf' });
    public lazy = false;
    public headers = [
        {
            text: '',
    ...
    public sgpValidaciones = {
        valor: [(v: any) => !!v || 'Este campo es obligatorio'],
        interes: [(v: any) => !!v || 'Este campo es obligatorio'],
        archivoValor: [
            (v: any) => {
        ...
        archivoInteres: [(v: any) => !!v || 'Este campo es obligatorio'],
        mes: [(v: any) => !!v || 'Este campo es obligatorio'],
    };
    public mounted() {
        const i = 0;
    ...
    public saveSgp() {
        console.log(
            (this.$refs.obs as Vue & { validate: () => boolean })
                .validate()
                .valueOf(),
        );
        this.sgp.fecha = new Date(
            this.fecha.getFullYear(),
            this.mes,
            this.fecha.getDay(),
        );
        this.sgp.valor = parseFloat(
            (Math.round(this.sgp.valor * 100) / 100).toString(),
        );
        this.sgp.interes = parseFloat(
            (Math.round(this.sgp.interes * 100) / 100).toString(),
        );
    ...
    public deleteItem(sgp: Isgp) {
        ...
        confirm('Estas seguro eliminar este dato?') &&
